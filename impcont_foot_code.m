...
ankle_trans_in.signals.values=repmat(ankle_trans_in.signals.values,nstep,1);
...
% Foot Translation and Angles
[foot_trans_in,foot_angles_in] = footkin(foot,nstep,pp);
foot_angles_in.signals.values = foot_angles_in.signals.values(:,[1 3]);
% af_angles_in.signals.values = foot_angles_in.signals.values;

% Foot Angular Velocity
foot_vel_in.signals.values = diff(foot_angles_in.signals.values);
foot_vel_in.signals.values = [foot_vel_in.signals.values; foot_vel_in.signals.values(end,:)];
foot_vel_in.signals.values = curve_smooth(foot_vel_in.signals.values);

% Foot Angular Acceleration
foot_acc_in.signals.values = diff(foot_vel_in.signals.values);
foot_acc_in.signals.values = [foot_acc_in.signals.values; foot_acc_in.signals.values(end,:)];
foot_acc_in.signals.values = curve_smooth(foot_acc_in.signals.values);

% Foot wrt Ankle
af_angles_in.signals.values = foot_angles_in.signals.values-ankle_angles_in.signals.values;
% Foot wrt Ankle Angular Velocity
af_vel_in.signals.values = diff(af_angles_in.signals.values);
af_vel_in.signals.values = [af_vel_in.signals.values; af_vel_in.signals.values(end,:)];
af_vel_in.signals.values = curve_smooth(af_vel_in.signals.values);

% Foot wrt Ankle Angular Acceleration
af_acc_in.signals.values = diff(af_vel_in.signals.values);
af_acc_in.signals.values = [af_acc_in.signals.values; af_acc_in.signals.values(end,:)];
af_acc_in.signals.values = curve_smooth(af_acc_in.signals.values);
...
% Ankle Acceleration
ankle_acc_in.signals.values = diff(ankle_vel_in.signals.values);
ankle_acc_in.signals.values = [ankle_acc_in.signals.values; ankle_acc_in.signals.values(end,:)];
ankle_acc_in.signals.values = curve_smooth(ankle_acc_in.signals.values);

% Shin Acceleration
shin_acc_in.signals.values = diff(shin_vel_in.signals.values);
shin_acc_in.signals.values = [shin_acc_in.signals.values; shin_acc_in.signals.values(end,:)];
shin_acc_in.signals.values = curve_smooth(shin_acc_in.signals.values);
...
shin_acc_in.time = ti;
ankle_acc_in.time = ti;

foot_angles_in.time = ti;
foot_trans_in.time = ti;
foot_vel_in.time = ti;
foot_acc_in.time = ti;
af_vel_in.time = ti;
af_acc_in.time = ti;
af_angles_in.time = ti;

q_des = [ankle_trans_in.signals.values shin_angles_in.signals.values];
qd_des = [ankle_vel_in.signals.values shin_vel_in.signals.values];
qdd_des = [ankle_acc_in.signals.values shin_acc_in.signals.values];
% X.signals.values = qdes;
X.signals.values = q_des(:,[1 3 2 6 4 5]);
% Xd.signals.values = qddes;
Xd.signals.values = qd_des(:,[1 3 2 6 4 5]);
Xdd.signals.values = qdd_des(:,[1 3 2 6 4 5]);

X.time = ankle_trans_in.time;
Xd.time = ankle_vel_in.time;
Xdd.time = ankle_acc_in.time;

% initpos = inv(jacob_gaitEmul2(zeros(6,1)))*[ankle_trans_in.signals.values(1,:).';zeros(3,1)];
% initpos = inv(jacob_gaitEmul2(zeros(6,1)))*X.signals.values(1,:).';
...
% set_param('shinhipfoot_imp', 'StopTime', 'ti(end)')
% % set_param('foot_imp','AlgebraicLoopSolver','LineSearch')
% sim('shinhipfoot_imp.slx');
% set_param('foot_imp', 'StopTime', 'ti(end)')
% set_param('foot_imp','AlgebraicLoopSolver','LineSearch')
% sim('foot_imp.slx');
% set_param('shinfoot', 'StopTime', 'ti(end)')
% sim('shinfoot.slx');
