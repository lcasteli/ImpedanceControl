...
% ankle_trans_in.signals.values(:,2)=ankle_trans_in.signals.values(:,2)+0.05;
...
set_param('pid_01', 'StopTime', 'ti(end)')
sim('pid_01.slx');
...
ankle_torque = repmat(ankle.torque,step,1); %DP/IE/ML
% smoothing
...
figure; plot(tf,tt_dp.signals.values)
hold on;
plot(tf,torque(:,3))
hold on;
plot(tf,-torque(:,1))
hold on;
plot(ti_tor,ankle_torque)
