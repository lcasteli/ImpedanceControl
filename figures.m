...
% Stiffness
figure; P1 = plot(1:length(Kdp),results.stiffness.kdplm); hold on;
P2 = plot(1:length(Kdp),Kdp,'--');
set(P1,'LineWidth',LW); set(P2,'Linewidth',LW+2)
xlim([1 20])
...
figure; P3 = plot(1:length(Kie),results.stiffness.kielm); hold on;
P4 = plot(1:length(Kie),Kie,'--');
xlim([1 20])
set(P3,'LineWidth',LW); set(P4,'Linewidth',LW+2)
ylabel('Stiffness (N-m/rad)')
xlabel('Simulation Number')
...
xlim([1 20]); ylim([0.96 1]);
ylabel('R^2')
xlabel('Simulation Number')
title('R^2 for Stiffness estimation in IE')
set(gca,'FontSize', FS);

% Torque Error at 319 and 119
figure; P5 = plot(tout,torque(:,3)); hold on;
P6 = plot(tt_dp.time,tt_dp.signals.values(:,3),':');
set(P5,'LineWidth',LW-0.5); set(P6,'Linewidth',LW)
xlabel('Simulation Time (secs)')
ylabel('Ankle Torque in DP (N-m)')
legend('Estimated Torque','Actual Torque','location','best')
set(gca,'FontSize', FS);

figure; P7 = plot(tout,-torque(:,1)); hold on;
P8 = plot(tt_ie.time,tt_ie.signals.values(:,3),':');
set(P7,'LineWidth',LW-0.5); set(P8,'Linewidth',LW)
xlabel('Simulation Time (secs)')
ylabel('Ankle Torque in IE (N-m)')
legend('Estimated Torque','Actual Torque','location','best')
set(gca,'FontSize', FS);
