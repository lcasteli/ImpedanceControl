function [qp, dev] = ik_test_no_y(t, q)
% Inverse Kinematics
%   t: coordinate target
...
%     end
    ...
    for k = 1 : ITER
        ...
        s = FK_test_no_y(q);          % current pose, as transf matrix
...
%     fprintf('Solution completed: %d steps required', k)
    qp = q;
    dev = t - FK_test_no_y(q); % deviation from solution
end
...
%     T23 = [0 0 1 0.0449; 0 1 0 0.1713; -1 0 0 0.5028; 0 0 0 1] * [eye(3)
%     [0 0 q(3)]'; 0 0 0 1]; % No_y
%     T34 = [0 0 -1 -0.032; -1 0 0 0.015; 0 1 0 0.2392; 0 0 0 1] * [Rz(q(4)) [0 0 0]'; 0 0 0 1];
    T24 = [0 1 0 0.2841; -1 0 0 0.1863; 0 0 1 0.5348; 0 0 0 1] * [Rz(q(3)) [0 0 0]'; 0 0 0 1];
    T45 = [0 0 -1 0.02; 0 -1 0 0; -1 0 0 -0.017; 0 0 0 1] * [Rz(q(4)) [0 0 0]'; 0 0 0 1];
    T56 = [0 -1 0 0; 0 0 1 -0.042; -1 0 0 0.02; 0 0 0 1] * [Rz(q(5)) [0 0 0]'; 0 0 0 1];
